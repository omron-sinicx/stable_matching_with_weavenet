...
from ..weavenet import TrainableMatchingModule, MatchingModuleHead, Unit, UnitListGenerator, ExclusiveElementsOfUnit, UnitProcOrder
from .layers import *
from ..layers import CrossConcat
from typing import List, Tuple, Optional

class TrainableMatchingModuleSp(TrainableMatchingModule):
    r""" A variant of :class:`TrainableMatchingModule <models.components.weavenet.TrainableMatchingModule>` that treats sparse bipartite graph.
    ...
    Args:
        ...
        output_channels: see :class:`TrainableMatchingModule <models.components.weavenet.TrainableMatchingModule>`.
        pre_interactor: :class:`TrainableMatchingModule <models.components.weavenet.TrainableMatchingModule>`.
        pre_head: a GNN pre_head applied before **mask_estimator**.
        stream_aggregator: the aggregator that merges estimation from all the streams (Default: :class:`DualSoftMaxSp <models.components.sparse.layers.DualSoftmaxSqrtSp>`)            
...
class MatchingModuleHeadSp(MatchingModuleHead):
    r""" A head of matching module. This controlls the way of interaction at each end of unit-process and residual paths. See :class:`MatchingModuleHead <models.components.weavenet.MatchingModuleHead>` for initialization.
...
class UnitSp(Unit):
    r""" a sparse version of :class:`Unit <models.components.weavenet.Unit>`.
...
class WeaveNetUnitListGeneratorSp(UnitListGenerator):
    r""" Sparse version of :class:`WeaveNetUnitListGenerator <models.components.weavenet.WeaveNetUnitListGenerator>`
...
class ExperimentalUnitListGeneratorSp(UnitListGenerator):
    r""" Sparse version of :class:`ExperimentalUnitListGenerator <models.components.weavenet.ExperimentalUnitListGenerator>`
...
class WeaveNetHeadSp(MatchingModuleHeadSp):
    r""" Sparse version of :class:`WeaveNetHead <models.components.weavenet.WeaveNetHead>`

        Args:
            input_channels: input_channels for the first unit (see :class:`WeaveNetUnitListGenerator <models.components.weavenet.WeaveNetUnitListGenerator>`).
            output_channels_list: output_channels for the units (see :class:`WeaveNetUnitListGenerator <models.components.weavenet.WeaveNetUnitListGenerator>`). 
            mid_channels_list: mid_channels for each point-net-based set encoders (see :class:`WeaveNetUnitListGenerator <models.components.weavenet.WeaveNetUnitListGenerator>`).
            calc_residual: see :class:`MatchingModuleHead <models.components.weavenet.MatchingModuleHead>`
            keep_first_var_after: see :class:`MatchingModuleHead <models.components.weavenet.MatchingModuleHead>`
            exclusive_elements_of_unit: see :class:`MatchingModuleHead <models.components.weavenet.MatchingModuleHead>`
            is_single_stream: see :class:`MatchingModuleHead <models.components.weavenet.MatchingModuleHead>`
...
class ExperimentalHeadSp(MatchingModuleHeadSp):
    r""" Sparse version of :obj:`ExperimentalHead <models.components.weavenet.ExperimentalHead>`

        Args:
            input_channels: input_channels for the first unit (see :class:`ExperimentalUnitListGenerator <models.components.weavenet.ExperimentalUnitListGenerator>`).
            output_channels_list: output_channels for the units (see :class:`ExperimentalUnitListGenerator <models.components.weavenet.ExperimentalUnitListGenerator>`). 
            mid_channels_list: mid_channels for each point-net-based set encoders (see :class:`ExperimentalUnitListGenerator <models.components.weavenet.ExperimentalUnitListGenerator>`).
            calc_residual: see :class:`MatchingModuleHead <models.components.weavenet.MatchingModuleHead>`
            keep_first_var_after: see :class:`MatchingModuleHead <models.components.weavenet.MatchingModuleHead>`
            exclusive_elements_of_unit: see :class:`MatchingModuleHead <models.components.weavenet.MatchingModuleHead>`
            is_single_stream: see :class:`MatchingModuleHead <models.components.weavenet.MatchingModuleHead>`
